...
export default function AlbumCarousel() {
    ...
    if (isClick) {
      ...
      albumElements.forEach((element) => {
        if (element.contains(e.target as Node)) {
          clickedIndex = parseInt(element.getAttribute('data-album-index') || '-1');
        }
      });
  ...
  const getAlbumStyle = (index: number): CSSProperties => {
    ...
    const shortestAngleOffset = angleOffset > Math.PI ? angleOffset - 2 * Math.PI : 
                               angleOffset < -Math.PI ? angleOffset + 2 * Math.PI : angleOffset;
    ...
    const additionalXSpacing = Math.sign(shortestAngleOffset) * spacingMultiplier * 30;
    ...
    return {
      ...
      transform: `translateX(${xPosition + additionalXSpacing + dragRotationOffset}px) translateZ(${zPosition}px) scale(${selectedScale}) rotateY(${rotation + dragRotationOffset}deg)`,
      transition: isDragging ? 'transform 0.1s ease-out' : 'all 0.5s ease-out',
      cursor: 'pointer',
      // Hide other albums completely when one is selected
      visibility: isCurrentAlbumSelected && !isSelected ? 'hidden' as const : 'visible' as const,
    };
  };
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (selectedAlbum !== null && carouselRef.current && !carouselRef.current.contains(e.target as Node)) {
        setSelectedAlbum(null);
      }
    };
  ...
  return (
    <div className="bg-primary/30 backdrop-blur-sm p-4 rounded-lg shadow-xl border border-white/10 mb-8 relative overflow-hidden">
      ...
      <h3 className="text-2xl font-lora font-semibold text-yellow mb-4 relative z-10">Nuestras Producciones</h3>
      
      <div 
        className="relative h-[400px] w-full overflow-hidden perspective-1000 my-4 z-10"
        ref={carouselRef}
      ...
      >
        <div className="absolute inset-0 flex items-center justify-center transform-style-3d">
          {albums.map((album, index) => {
            ...
            const shouldRender = selectedAlbum === index || 
                               Math.abs((index - currentIndex + albums.length) % albums.length) <= 5 || 
                               Math.abs((currentIndex - index + albums.length) % albums.length) <= 5;
            ...
            return (
              <div
                key={album.id}
                className={`absolute w-56 h-56 transition-all transform-gpu ${isSelected ? 'selected-album' : ''}`}
                style={getAlbumStyle(index)}
                aria-hidden={index !== currentIndex && !isSelected}
                data-album-index={index}
              >
                <div className={`relative w-full h-full rounded-lg overflow-hidden ${index === currentIndex && !selectedAlbum ? 'ring-4 ring-yellow/70' : ''} ${isSelected ? 'ring-4 ring-yellow' : ''}`}>
                  ...
                  <Image
                    ...
                    priority={index === currentIndex || isSelected || Math.abs(index - currentIndex) <= 2}
                  />
                </div>
              </div>
            );
      ...
      <div className="text-center mt-2 h-16 relative z-10">
        <h4 className="text-xl font-lora font-semibold text-yellow">
          {selectedAlbum !== null ? albums[selectedAlbum].title : albums[currentIndex].title}
        </h4>
        <p className="font-mono text-white/80">
          {selectedAlbum !== null ? albums[selectedAlbum].artist : albums[currentIndex].artist}
        </p>
      </div>
